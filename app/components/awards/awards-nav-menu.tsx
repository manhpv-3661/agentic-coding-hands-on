import type { AwardCategory } from "@/lib/awards/award-categories";
import { GOLD_GLOW_TEXT_SHADOW } from "@/lib/ui/gold-glow";

export interface AwardsNavMenuProps {
  /** Categories to render, in display order (FR-6). Passed as a prop rather
   * than imported directly so this component stays pure and trivially
   * testable — the caller (Phase 05 `awards-catalog`) supplies
   * `AWARD_CATEGORIES`. */
  items: AwardCategory[];
  /** Currently active category slug, driven by the scroll-spy hook owned by
   * the catalog (Phase 01). `null`/unknown slug → nothing highlighted, no
   * throw (FR-10). */
  activeSlug: string | null;
  /** Nav landmark aria-label (`shared.a11y.awardCategories`) —
   * optional/defaulted to the English design label so existing
   * callers/tests that predate this prop keep compiling unchanged. */
  ariaLabel?: string;
}

/**
 * Target leading icon, MoMorph asset `public/awards-saa/Icon-Target.svg`,
 * inlined (not `<img>`) with `fill="currentColor"` so it tints gold along
 * with the label when its parent link is active (FR-8).
 */
function IconTarget({ className }: { className?: string }) {
  return (
    <svg
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      aria-hidden="true"
    >
      <path
        d="M12 2C10.6868 2 9.38642 2.25866 8.17317 2.7612C6.95991 3.26375 5.85752 4.00035 4.92893 4.92893C3.05357 6.8043 2 9.34784 2 12C2 14.6522 3.05357 17.1957 4.92893 19.0711C5.85752 19.9997 6.95991 20.7362 8.17317 21.2388C9.38642 21.7413 10.6868 22 12 22C14.6522 22 17.1957 20.9464 19.0711 19.0711C20.9464 17.1957 22 14.6522 22 12C22 10.84 21.79 9.69 21.39 8.61L19.79 10.21C19.93 10.8 20 11.4 20 12C20 14.1217 19.1571 16.1566 17.6569 17.6569C16.1566 19.1571 14.1217 20 12 20C9.87827 20 7.84344 19.1571 6.34315 17.6569C4.84285 16.1566 4 14.1217 4 12C4 9.87827 4.84285 7.84344 6.34315 6.34315C7.84344 4.84285 9.87827 4 12 4C12.6 4 13.2 4.07 13.79 4.21L15.4 2.6C14.31 2.21 13.16 2 12 2ZM19 2L15 6V7.5L12.45 10.05C12.3 10 12.15 10 12 10C11.4696 10 10.9609 10.2107 10.5858 10.5858C10.2107 10.9609 10 11.4696 10 12C10 12.5304 10.2107 13.0391 10.5858 13.4142C10.9609 13.7893 11.4696 14 12 14C12.5304 14 13.0391 13.7893 13.4142 13.4142C13.7893 13.0391 14 12.5304 14 12C14 11.85 14 11.7 13.95 11.55L16.5 9H18L22 5H19V2ZM12 6C10.4087 6 8.88258 6.63214 7.75736 7.75736C6.63214 8.88258 6 10.4087 6 12C6 13.5913 6.63214 15.1174 7.75736 16.2426C8.88258 17.3679 10.4087 18 12 18C13.5913 18 15.1174 17.3679 16.2426 16.2426C17.3679 15.1174 18 13.5913 18 12H16C16 13.0609 15.5786 14.0783 14.8284 14.8284C14.0783 15.5786 13.0609 16 12 16C10.9391 16 9.92172 15.5786 9.17157 14.8284C8.42143 14.0783 8 13.0609 8 12C8 10.9391 8.42143 9.92172 9.17157 9.17157C9.92172 8.42143 10.9391 8 12 8V6Z"
        fill="currentColor"
      />
    </svg>
  );
}

/**
 * Vertical award-category nav menu — MoMorph node ties to the "Danh mục"
 * sidebar in the awards catalog layout. Pure function of props: no client
 * hook, no internal click-pinned state, so it does not need `"use client"`.
 *
 * Active highlighting is driven purely by the `activeSlug` prop (from the
 * real IntersectionObserver scroll-spy owned by Phase 01/05) — clicking an
 * item is a native anchor jump (`href="#<slug>"`); the observer settles the
 * active item afterwards. This also makes `/awards#<slug>` hash-deep-links
 * from the homepage work for free (FR-7, clarification 2026-07-06).
 *
 * Active state (gold `#FFEA9E` text + bottom border + glow) mirrors the
 * site header's active-link treatment (`app/components/home/nav-link.tsx`)
 * exactly — same border-bottom + `textShadow` values — per the MoMorph
 * ground truth for this screen (`313:8459`, item C.1 "Top Talent").
 */
export function AwardsNavMenu({ items, activeSlug, ariaLabel = "Award categories" }: AwardsNavMenuProps) {
  return (
    // mm:awards-nav-menu
    <nav aria-label={ariaLabel} className="sticky top-24">
      <ul className="flex flex-col gap-4">
        {items.map(({ slug, title }) => {
          const isActive = slug === activeSlug;
          return (
            <li key={slug}>
              <a
                href={`#${slug}`}
                aria-current={isActive || undefined}
                className={`font-montserrat flex items-center gap-1 rounded-[4px] p-4 text-sm leading-5 font-bold tracking-[0.25px] transition-colors duration-200 ease-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#FFEA9E] ${
                  isActive
                    ? "border-b border-[#FFEA9E] text-[#FFEA9E]"
                    : "text-white hover:bg-white/10"
                }`}
                style={isActive ? { textShadow: GOLD_GLOW_TEXT_SHADOW } : undefined}
              >
                <IconTarget className="h-6 w-6 shrink-0" />
                <span>{title}</span>
              </a>
            </li>
          );
        })}
      </ul>
    </nav>
  );
}
