...
Route này hiện là placeholder tối giản từ F002 (mỗi hạng mục chỉ có `<section id={slug}>` rỗng) — F004
...
- **FR-16**: `app/awards/page.tsx` PHẢI tự render `SiteHeader`/`SiteFooter` (giống `app/page.tsx`) —
  layout gốc (`app/layout.tsx`) chỉ render html/body, không tự cấp header/footer. Placeholder hiện
  tại thiếu điều này; đây là 1 phần cần bổ sung của F004, không phải regression.

## 5. Unresolved Questions
(none — 3 điểm mơ hồ đã được làm rõ, xem `../../clarifications.md`; mô tả dài D.1–D.6 đã bổ sung
verbatim ở mục 2.5 sau khi planner phát hiện thiếu ở lần soạn spec đầu)
